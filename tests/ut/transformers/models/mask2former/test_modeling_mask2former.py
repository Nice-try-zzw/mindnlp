...
class Mask2FormerModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    @unittest.skip(reason="Mask2Former does not have a base model")
    def test_backbone_selection(self):
        # from huggingface_hub import login
        # login('account token', add_to_git_credential=True)
        config, inputs = self.model_tester.prepare_config_and_inputs_for_common()
...
class Mask2FormerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        self.assertTrue(
            np.allclose(
                outputs.encoder_last_hidden_state[0, 0, :3, :3].asnumpy(), expected_slice_hidden_state.asnumpy(), atol=TOLERANCE
            )
        )
        ...
        self.assertTrue(
            np.allclose(
                outputs.pixel_decoder_last_hidden_state[0, 0, :3, :3].asnumpy(), expected_slice_hidden_state.asnumpy(), atol=TOLERANCE
            )
        )
        ...
        self.assertTrue(
            np.allclose(
                outputs.transformer_decoder_last_hidden_state[0, :3, :3].asnumpy(), expected_slice_hidden_state.asnumpy(), atol=TOLERANCE
            )
        )
    ...
    def test_inference_universal_segmentation_head(self):
        ...
        self.assertTrue(np.allclose(masks_queries_logits[0, 0, :3, :3].asnumpy(), expected_slice.asnumpy(), atol=TOLERANCE))
        ...
        self.assertTrue(np.allclose(outputs.class_queries_logits[0, :3, :3].asnumpy(), expected_slice.asnumpy(), atol=TOLERANCE))

    def test_inference_fp16(self):
        model = (
            Mask2FormerForUniversalSegmentation.from_pretrained(self.model_checkpoints, ms_dtype=mindspore.float16)
            .set_train(False)
        )
        image_processor = self.default_image_processor
        image = prepare_img()
        inputs = image_processor(image, return_tensors="ms").to(mindspore.float16)
