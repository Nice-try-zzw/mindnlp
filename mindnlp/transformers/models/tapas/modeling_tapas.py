...
class TapasCategoricalMixture:

    """
    TapasCategoricalMixture represents a categorical mixture distribution for probability calculations based on given logits.
    ...
    def __init__(self, param, bins=50, start=0, end=1):
        """ 
        Initializes an instance of the TapasCategoricalMixture class.
        
        Args:
            self: Instance of the TapasCategoricalMixture class.
    ...
    def log_prob(self, true):
        ...
        Args:
            self: TapasCategoricalMixture
                The instance of the TapasCategoricalMixture class.
            true: torch.Tensor
...
range of valid classes for the categorical mixture model.
        ...
        Returns:
            None: 
                This method does not return any value. The log probability is calculated and stored internally within the TapasCategoricalMixture instance.
        ...
        nll = ops.log_softmax(self.logits)
    ...
    def construct(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if position_ids is None:
            ...
            if self.config.reset_position_index_per_cell:
                ...
                # shape (1, seq_len)
                position = ops.arange(
                    seq_length, dtype=mindspore.int64).unsqueeze(0)
                position_ids = ops.minimum(
                    mindspore.Tensor(
                        self.config.max_position_embeddings - 1), position - first_position
                )
...
class TapasEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([TapasLayer(config)
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, nn.Dense):
            ...
            cell.weight.set_data(initializer(
                Normal(self.config.initializer_range), cell.weight.shape, cell.weight.dtype))
            if cell.has_bias:
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    def __init__(self, config: TapasConfig):
        ...
        if config.init_cell_selection_weights_to_zero:
            ...
            self.output_weights = Parameter(
                ops.zeros(config.hidden_size), "output_weights")
            self.column_output_weights = Parameter(
                ops.zeros(config.hidden_size), "column_output_weights")
        else:
            self.output_weights = Parameter(mindspore.Tensor(
                np.empty(config.hidden_size)), "output_weights")
            self.output_weights.set_data(initializer(Normal(
                config.initializer_range), self.output_weights.shape, self.output_weights.dtype))
            self.column_output_weights = Parameter(mindspore.Tensor(
                np.empty(config.hidden_size)), "column_output_weights")
            self.column_output_weights.set_data(initializer(Normal(
                config.initializer_range), self.column_output_weights.shape, self.column_output_weights.dtype))
        self.output_bias = Parameter(ops.zeros([]), "output_bias")
        self.column_output_bias = Parameter(
            ops.zeros([]), "column_output_bias")

        # aggregation head
        if config.num_aggregation_labels > 0:
            self.aggregation_classifier = nn.Dense(
    ...
    def construct(
        ...
        row_index = IndexMap(
            indices=ops.minimum(row_ids, mindspore.Tensor(
                self.config.max_num_rows - 1)),
            num_segments=self.config.max_num_rows,
            batch_dims=1,
        )
        col_index = IndexMap(
            indices=ops.minimum(column_ids, mindspore.Tensor(
                self.config.max_num_columns - 1)),
        ...
        if table_mask is None:
            table_mask = ops.where(row_ids > 0, ops.ones_like(
                row_ids), ops.zeros_like(row_ids))
        ...
        if labels is not None:
            ...
            dist_per_token = nn.probability.distribution.Bernoulli(logits)
            ...
            if self.config.num_aggregation_labels > 0:
                if is_supervised:
                    ...
                    aggregation_labels = ops.zeros(
                        labels.shape[0], dtype=mindspore.int64)
                    per_example_additional_loss = _calculate_aggregation_loss(
                        logits_aggregation,
                        aggregate_mask,
                        aggregation_labels,
    ...
    def __init__(self, indices, num_segments, batch_dims=0):
        ...
        Args:
            ...
            batch_dims (`int`, *optional*, defaults to 0):
                ...
                index.
        """
        self.indices = mindspore.Tensor(indices)
        self.num_segments = mindspore.Tensor(num_segments)
        self.batch_dims = batch_dims

    def batch_shape(self):
        # returns a torch.Size object
        return self.indices.shape[: self.batch_dims]
...
class ProductIndexMap(IndexMap):
    ...
    def project_outer(self, index):
        ...
        indices = ops.div(index.indices, self.inner_index.num_segments,
                          rounding_mode="floor").type(mindspore.int64)
        return IndexMap(indices=indices, num_segments=self.outer_index.num_segments, batch_dims=index.batch_dims)

    def project_inner(self, index):
        """Projects an index with the same index set onto the inner components."""
        return IndexMap(
            indices=ops.fmod(index.indices.astype(mindspore.int32),
                             self.inner_index.num_segments.astype(mindspore.int32))
...
def gather(values, index, name="segmented_gather"):
    ...
    if len(values.shape[index.batch_dims:]) < 2:
        return ops.gather_d(values, index.batch_dims, indices)
    else:
...
def flatten(index, name="segmented_flatten"):
    ...
    *num_segments* * (k - 1). The result is a tensor with *num_segments* multiplied by the number of elements in the
    ...
    if len(index.batch_shape()) == 0:
        batch_size = mindspore.Tensor([1], dtype=mindspore.int64)
    else:
    ...
    offset = ops.arange(start=0, end=batch_size.item()) * index.num_segments
...
def range_index_map(batch_shape, num_segments, name="range_index_map"):
    ...
    num_segments = mindspore.Tensor(num_segments)
...
def _segment_reduce(values, index, segment_reduce_fn, name):
    ...
    try:
        flat_values = values.view(tuple(flattened_shape.tolist()))
    except:
        raise ValueError("Error in the shape of the values tensor.", values.shape, flattened_shape, values.dtype)

    out = ops.zeros(int(flat_index.num_segments), dtype=mindspore.float32)

    def mindspore_scatter_reduce(input, dim, index, src, reduce, include_self):
        if dim != 0:
            raise NotImplementedError("This implementation currently supports only dim=0")
        # Initialize the output tensor
        if include_self:
            output = input.clone()
        else:
            output = ops.zeros_like(input)
        # Handle different reduce operations
        if reduce == 'sum':
            for idx, val in zip(index, src):
                output[idx] += val if include_self else val - input[idx]
        elif reduce == 'amax':
            for idx, val in zip(index, src):
                output[idx] = max(output[idx], val) if include_self else val
        elif reduce == 'amin':
            for idx, val in zip(index, src):
                output[idx] = min(output[idx], val) if include_self else val
        elif reduce == 'mean':
            count = ops.zeros_like(input, dtype=input.dtype)
            for idx, val in zip(index, src):
                output[idx] += val
                count[idx] += 1
            non_zero = count != 0
            output[non_zero] /= count[non_zero]
            if include_self:
                output += input / (count + 1)
        else:
            raise ValueError(f"Reduction operation '{reduce}' is not supported.")
        return output
    segment_means = mindspore_scatter_reduce(
...
def compute_column_logits(
    ...
    is_padding = ops.logical_and(
        cell_count < 0.5, ~ops.eq(out_index.indices, 0))
    column_logits += CLOSE_ENOUGH_TO_LOG_ZERO * mindspore.Tensor(
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    ...
    labels_per_column, _ = reduce_sum(mindspore.Tensor(
        labels, dtype=mindspore.float32), col_index)
    ...
    column_label = ops.where(
        no_cell_selected.view(column_label.shape), ops.zeros_like(
            column_label), column_label
    )

    column_dist = TapasCategoricalMixture(
        column_logits)  # shape (batch_size, max_num_cols)
    ...
    labels_per_cell, labels_index = reduce_max(
        mindspore.Tensor(labels, dtype=mindspore.int64), cell_index
    )
    ...
    cell_dist = nn.probability.distribution.Bernoulli(
        logits_per_cell)  # shape (batch_size, 64*32)
    cell_log_prob = cell_dist.log_prob(labels_per_cell.type(
        mindspore.float32))  # shape(batch_size, 64*32)
    ...
    cell_loss /= ops.sum(column_mask * cell_mask, dim=1) + \
    ...
    selected_column_id = mindspore.Tensor(
    ...
    selected_column_mask = mindspore.Tensor(
...
def compute_token_logits(sequence_output, temperature, output_weights, output_bias):
    ...
    """
    logits = (ops.einsum("bsj,j->bs", sequence_output,
...
def _calculate_aggregate_mask(answer, pooled_output, cell_selection_preference, labels, aggregation_classifier):
    ...
    aggregate_mask_init = ops.logical_not(
        ops.isnan(answer)).type(mindspore.Tensor)
    logits_aggregation = aggregation_classifier(pooled_output)
    dist_aggregation = TapasCategoricalMixture(
        logits_aggregation)
    ...
    aggregate_mask = ops.where(
        ops.logical_and(is_pred_cell_selection, is_cell_supervision_available).view(
            aggregate_mask_init.shape),
...
def _calculate_aggregation_loss_known(
    ...
    if use_answer_as_supervision:
        ...
        target_aggregation = ops.zeros_like(
            aggregate_mask, dtype=mindspore.int64)
    else:
        # Use aggregation supervision as the target.
        target_aggregation = aggregation_labels

    one_hot_labels = ops.one_hot(
        target_aggregation, num_aggregation_labels).astype(mindspore.float32)
    log_probs = ops.log_softmax(logits_aggregation, axis=-1)

    # mindspore.Tensor[batch_size]
    per_example_aggregation_intermediate = - \
        ops.sum(one_hot_labels * log_probs, dim=-1)
    if use_answer_as_supervision:
...
def _calculate_expected_result(
    ...
    Args:
        dist_per_cell (`nn.probability.distribution.Bernoulli`):
    ...
    numeric_values_masked = ops.where(
        ops.isnan(numeric_values), ops.zeros_like(
            numeric_values), numeric_values
    )  # Mask non-numeric table values to zero.
    sum_result = ops.sum(scaled_probability_per_cell *
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        ex = ops.sum(scaled_probability_per_cell, dim=1,
                     keepdim=True) - scaled_probability_per_cell + 1
        average_result = ops.sum(
...
def _calculate_regression_loss(
    ...
    # float32 (batch_size,)
    answer_masked = ops.where(
        ops.isnan(answer), ops.zeros_like(answer), answer)

    if config.use_normalized_answer_loss:
        normalizer = (ops.max(ops.abs(expected_result), ops.abs(
    ...
    if config.answer_loss_cutoff is None:
        large_answer_loss_mask = ops.ones_like(
            per_example_answer_loss, dtype=mindspore.float32)
