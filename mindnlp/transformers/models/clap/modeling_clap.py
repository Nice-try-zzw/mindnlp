...
class ClapAudioAFFBlock(nn.Cell):
    ...
    def __init__(self, config: ClapAudioConfig):
        ...
        self.local_att = nn.SequentialCell(
            nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0, pad_mode="valid"),
            nn.BatchNorm2d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(inter_channels, channels, kernel_size=1, stride=1, padding=0, pad_mode="valid"),
...
class ClapAudioLayer(nn.Cell):
    ...
    def get_attn_mask(self, height, width, dtype, device):
        if self.shift_size > 0:
            ...
            img_mask = ops.zeros((1, height, width, 1), dtype=dtype, device=device)
            height_slices = (
...
class ClapAudioEncoder(nn.Cell):
    ...
    def construct(
        ...
        if self.enable_fusion:
            ...
            is_longer_list_idx = ops.argwhere(is_longer_list == 1)[0]
...
class ClapTextEmbeddings(nn.Cell):
    ...
    def create_position_ids_from_inputs_embeds(self, inputs_embeds):
        ...
        position_ids = ops.arange(
            self.padding_idx + 1, sequence_length + self.padding_idx + 1, dtype=mindspore.int64, device=inputs_embeds.device
        )
...
class ClapTextSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if use_cache:
                position_ids_l = mindspore.Tensor(key_length - 1, dtype=mindspore.int64, device=hidden_states.device).view(
                    -1, 1
                )
            else:
                position_ids_l = torch.arange(query_length, dtype=mindspore.int64, device=hidden_states.device).view(-1, 1)
            position_ids_r = torch.arange(key_length, dtype=mindspore.int64, device=hidden_states.device).view(1, -1)
    ...
    def construct(
        ...
        if token_type_ids is None:
            if hasattr(self.embeddings, "token_type_ids"):
                ...
                token_type_ids = torch.zeros(input_shape, dtype=mindspore.int64, device=device)
