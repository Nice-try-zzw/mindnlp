...
def window_partition(input_feature, window_size):
    ...
    windows = input_feature.permute(
        0, 1, 3, 2, 4, 5).view(-1, window_size, window_size, num_channels)
    return windows
...
def window_reverse(windows, window_size, height, width):
    ...
    windows = windows.permute(
        0, 1, 3, 2, 4, 5).view(-1, height, width, num_channels)
    return windows
...
class SwinEmbeddings(nn.Cell):
    ...
    def __init__(self, config, use_mask_token=False):
        ...
        self.mask_token = Parameter(
            ops.zeros(1, 1, config.embed_dim), 'mask_token') if use_mask_token else None

        if config.use_absolute_embeddings:
            self.position_embeddings = Parameter(
                ops.zeros(1, num_patches + 1, config.embed_dim), 'position_embeddings')
        else:
    ...
    def construct(
        ...
        if bool_masked_pos is not None:
            ...
            mask = bool_masked_pos.unsqueeze(-1).astype(mask_tokens)
...
class SwinSelfAttention(nn.Cell):
    def __init__(self, config, dim, num_heads, window_size):
        ...
        self.relative_position_bias_table = Parameter(
            ops.zeros((2 * self.window_size[0] - 1) *
        ...
        self.query = nn.Dense(self.all_head_size,
                              self.all_head_size, has_bias=config.qkv_bias)
...
class SwinLayer(nn.Cell):
    def __init__(self, config, dim, input_resolution, num_heads, shift_size=0):
        ...
        self.layernorm_before = nn.LayerNorm(
            [dim], epsilon=config.layer_norm_eps)
    ...
    def construct(
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_hidden_states = mindspore.numpy.roll(
        ...
        if self.shift_size > 0:
            attention_windows = mindspore.numpy.roll(shifted_windows, shift=(
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, (nn.Dense, nn.Conv2d)):
            ...
            cell.weight.set_data(initializer(Normal(
                self.config.initializer_range), cell.weight.data.shape, cell.weight.data.dtype))
            if cell.has_bias:
                cell.bias.set_data(initializer(
                    'zeros', cell.bias.data.shape, cell.bias.data.dtype))
        elif isinstance(cell, nn.LayerNorm):
            cell.bias.set_data(initializer(
                'zeros', cell.bias.data.shape, cell.bias.data.dtype))
            cell.weight.set_data(initializer(
                'ones', cell.weight.data.shape, cell.weight.data.dtype))
...
class SwinForMaskedImageModeling(SwinPreTrainedModel):
    ...
    def construct(
        ...
        if bool_masked_pos is not None:
            ...
            reconstruction_loss = ops.l1_loss(
                pixel_values, reconstructed_pixel_values, reduction="none")
            masked_im_loss = (reconstruction_loss * mask).sum() / \
                (mask.sum() + 1e-5) / self.config.num_channels
...
class SwinForImageClassification(SwinPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier head
        self.classifier = (
            nn.Dense(self.swin.num_features,
