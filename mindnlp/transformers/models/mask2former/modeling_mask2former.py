...
def sample_point(
    ...
    # use ops.grid_sample to get features for points in `point_coordinates` via bilinear interpolation
    point_features = ops.grid_sample(
...
def pair_wise_sigmoid_cross_entropy_loss(inputs: mindspore.Tensor, labels: mindspore.Tensor) -> mindspore.Tensor:
    ...
    cross_entropy_loss_pos = ops.binary_cross_entropy_with_logits(inputs, ops.ones_like(inputs), reduction="none")
    cross_entropy_loss_neg = ops.binary_cross_entropy_with_logits(inputs, ops.ones_like(inputs), reduction="none")

    loss_pos = ops.matmul(cross_entropy_loss_pos /
                            height_and_width, labels.T)
    loss_neg = ops.matmul(cross_entropy_loss_neg /
                            height_and_width, (1 - labels).T)
...
class Mask2FormerHungarianMatcher(nn.Cell):
    ...
    def construct(
        ...
        for i in range(batch_size):
            ...
            # final cost matrix
            cost_matrix = self.cost_mask * cost_mask + \
                self.cost_class * cost_class + self.cost_dice * cost_dice
        ...
        matched_indices = [
            (mindspore.Tensor(i, dtype=mindspore.int64), mindspore.Tensor(j, dtype=mindspore.int64)) for i, j in indices
        ]
...
class Mask2FormerLoss(nn.Cell):
    ...
    def _pad_images_to_max_in_batch(self, tensors: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        padding_masks = ops.ones(
            (batch_size, height, width), dtype=mindspore.bool_)
        # pad the tensors to the size of the biggest one
        for tensor, padded_tensor, padding_mask in zip(tensors, padded_tensors, padding_masks):
            padded_tensor[: tensor.shape[0], : tensor.shape[1],
                          : tensor.shape[2]] = tensor
    ...
    def loss_labels(
        ...
        idx = self._get_predictions_permutation_indices(
            indices)  # shape of (batch_size, num_queries)
        target_classes_o = ops.cat(
            [target[j] for target, (_, j) in zip(class_labels, indices)]
        )  # shape of (batch_size, num_queries)
        target_classes = ops.full(
            (batch_size, num_queries), fill_value=self.num_labels, dtype=mindspore.int64
        )
        ...
        loss_ce = ops.cross_entropy(pred_logits_transposed, target_classes, weight=self.empty_weight)
    ...
    def loss_masks(
        ...
        point_labels = sample_point(
            target_masks, point_coordinates, align_corners=False).squeeze(1)

        point_logits = sample_point(
            pred_masks, point_coordinates, align_corners=False).squeeze(1)
    ...
    def _get_predictions_permutation_indices(self, indices):
        ...
        batch_indices = ops.cat([ops.full_like(src, i)
    ...
    def _get_targets_permutation_indices(self, indices):
        ...
        batch_indices = ops.cat([ops.full_like(tgt, i)
    ...
    def sample_points_using_uncertainty(
        ...
        point_logits = sample_point(
            logits, point_coordinates, align_corners=False)
        ...
        shift = num_points_sampled * \
            ops.arange(num_boxes, dtype=mindspore.int64)
        idx += shift[:, None]
    ...
    def construct(
        ...
        indices = self.matcher(masks_queries_logits,
                               class_queries_logits, mask_labels, class_labels)
        # compute the average number of target masks for normalization purposes
        num_masks = self.get_num_masks(
            class_labels)
        # get all the losses
        ...
        if auxiliary_predictions is not None:
            for idx, aux_outputs in enumerate(auxiliary_predictions):
                ...
                loss_dict = {f"{key}_{idx}": value for key,
                             value in loss_dict.items()}
    ...
    def get_num_masks(self, class_labels: mindspore.Tensor) -> mindspore.Tensor:
        ...
        """
        num_masks = mindspore.Tensor(
            ops.reduce_sum(mindspore.Tensor([len(lab) for lab in class_labels])), dtype=mindspore.float32)
        world_size = 1
...
def multi_scale_deformable_attention(
    ...
    value_list = value.split([height.item() * width.item()
                             for height, width in value_spatial_shapes], axis=1)
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        # -> batch_size*num_heads, hidden_dim, height, width
        value_l_ = (
            value_list[level_id].flatten(start_dim=2).swapaxes(1, 2).reshape(
                batch_size * num_heads, hidden_dim, height.asnumpy().item(), width.asnumpy().item())
        )
        # batch_size, num_queries, num_heads, num_points, 2
        # -> batch_size, num_heads, num_queries, num_points, 2
        # -> batch_size*num_heads, num_queries, num_points, 2
        sampling_grid_l_ = sampling_grids[:, :, :, level_id].swapaxes(
            1, 2).flatten(start_dim=0, end_dim=1)
...
class Mask2FormerSinePositionEmbedding(nn.Cell):
    ...
    def construct(self, x: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        if mask is None:
            mask = ops.zeros((x.shape[0], x.shape[2], x.shape[3]),
                               dtype=mindspore.bool_)
        not_mask = (~mask).astype(x.dtype)
        ...
        dim_t = ops.arange(self.num_pos_feats,
                             dtype=mindspore.int64).type_as(x)
        ...
        pos_x = ops.stack(
            (pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), axis=4).flatten(start_dim=3)
        pos_y = ops.stack(
            (pos_y[:, :, :, 0::2].sin(), pos_y[:, :, :, 1::2].cos()), axis=4).flatten(start_dim=3)
    ...
    def get_reference_points(spatial_shapes, valid_ratios):
        ...
        for lvl, (height, width) in enumerate(spatial_shapes):
            ref_y, ref_x = ops.meshgrid(
                ops.linspace(0.5, height - 0.5, height).astype(valid_ratios.dtype),
                ops.linspace(0.5, width - 0.5, width).astype(valid_ratios.dtype),
                indexing="ij",
            )
...
class Mask2FormerPixelDecoder(nn.Cell):
    def __init__(self, config: Mask2FormerConfig, feature_channels):
        ...
        self.level_embed = Parameter(
            ops.zeros([self.num_feature_levels, feature_dim]), 'level_embed')
        ...
        for idx, in_channels in enumerate(self.feature_channels[: self.num_fpn_levels]):
            lateral_conv = nn.SequentialCell(
                nn.Conv2d(in_channels, feature_dim, kernel_size=1, has_bias=False, pad_mode='pad'),
                nn.GroupNorm(32, feature_dim),
            )

            output_conv = nn.SequentialCell(
                nn.Conv2d(feature_dim, feature_dim, kernel_size=3,
                          stride=1, padding=1, has_bias=False, pad_mode='pad'),
                nn.GroupNorm(32, feature_dim),
                nn.ReLU(),
            )
            setattr(self, "adapter_{}".format(idx + 1), lateral_conv)
            setattr(self, "layer_{}".format(idx + 1), output_conv)
    ...
    def get_valid_ratio(self, mask, dtype=mindspore.float32):
        ...
        valid_ratio_heigth = valid_height.astype(dtype) / height
    ...
    def construct(
        ...
        spatial_shapes = [(embed.shape[2], embed.shape[3])
                          for embed in input_embeds]
        input_embeds_flat = ops.cat(
            [embed.flatten(start_dim=2).swapaxes(1, 2) for embed in input_embeds], 1)
        spatial_shapes = mindspore.Tensor(
            spatial_shapes, dtype=mindspore.int64)
        masks_flat = ops.cat([mask.flatten(start_dim=1) for mask in masks], 1)

        position_embeddings = [embed.flatten(start_dim=2).swapaxes(
            1, 2) for embed in position_embeddings]
        try:
            level_pos_embed_flat = [
                x + self.level_embed[i].view(1, 1, -1) for i, x in enumerate(position_embeddings)]
        except:
            raise ValueError(
                "The number of feature levels should match the number of level embeddings",
                len(position_embeddings),
                self.level_embed.shape,
            )
        level_pos_embed_flat = ops.cat(level_pos_embed_flat, 1)

        level_start_index = ops.cat(
            (spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        valid_ratios = ops.stack([self.get_valid_ratio(
            mask, dtype=input_embeds_flat.dtype) for mask in masks], 1)
        ...
        for i in range(self.num_feature_levels):
            if i < self.num_feature_levels - 1:
                split_sizes[i] = level_start_index[i + 1] - \
                    level_start_index[i]
            else:
                split_sizes[i] = last_hidden_state.shape[1] - \
                    level_start_index[i]

        encoder_output = ops.split(
            last_hidden_state, [size.item() for size in split_sizes], axis=1)

        # Compute final features
        outputs = [
            x.swapaxes(1, 2).view(batch_size, -1,
                                   spatial_shapes[i][0].asnumpy().item(), spatial_shapes[i][1].asnumpy().item())
...
class Mask2FormerPixelLevelModule(nn.Cell):
    ...
    def construct(self, pixel_values: Tensor, output_hidden_states: bool = False) -> Mask2FormerPixelLevelModuleOutput:
        ...
        return Mask2FormerPixelLevelModuleOutput(
            ...
            encoder_hidden_states=tuple(
                backbone_features) if output_hidden_states else None,
...
class Mask2FormerAttention(nn.Cell):
    ...
    def construct(
        ...
        key_value_position_embeddings = (
            key_value_position_embeddings.permute(
...
class Mask2FormerMaskedAttentionDecoderLayer(nn.Cell):
    ...
    def forward_post(
        ...
        hidden_states, cross_attn_weights = self.cross_attn(
            self.with_pos_embed(
                hidden_states, query_position_embeddings),
            self.with_pos_embed(
...
class Mask2FormerMaskedAttentionDecoder(nn.Cell):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.layers = nn.CellList(
            [Mask2FormerMaskedAttentionDecoderLayer(
                self.config) for _ in range(self.decoder_layers)]
        )
    ...
    def construct(
        ...
        Args:
            ...
            feature_size_list (`List[mindspore.shape]` ):
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                all_true_indices = ops.argwhere(sum_mask == attention_mask.shape[-1])
                attention_mask_np = attention_mask.asnumpy()
                for indice in all_true_indices:
                    attention_mask_np[indice.asnumpy().astype(int)[0], indice.asnumpy().astype(int)[1], :] = False
                attention_mask = Tensor(attention_mask_np, dtype=mindspore.bool_)
        ...
        if not return_dict:
            outputs = [hidden_states, all_hidden_states, attentions,
                       intermediate, intermediate_mask_predictions]
...
class Mask2FormerMLPPredictionHead(nn.Cell):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, num_layers: int = 3):
        ...
        for i, (in_dim, out_dim) in enumerate(zip(in_dims, out_dims)):
            ...
            layer = Mask2FormerPredictionBlock(
                in_dim, out_dim, activation=activation)
...
class Mask2FormerMaskPredictor(nn.Cell):
    ...
    def construct(self, outputs: mindspore.Tensor, pixel_embeddings: mindspore.Tensor, attention_mask_target_size: int = None):
        ...
        attention_mask = (attention_mask.flatten(start_dim=0, end_dim=1) < 0.5).astype(mindspore.bool_)
    ...
    def construct(
        ...
        for i in range(self.num_feature_levels):
            ...
            multi_stage_positional_embeddings.append(
                self.position_embedder(multi_scale_features[i], None).flatten(start_dim=2))
...
class Mask2FormerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell: nn.Cell):
        ...
        if isinstance(cell, Mask2FormerTransformerModule):
            if cell.input_projections is not None:
                for input_projection in cell.input_projections:
                    if not isinstance(input_projection, nn.SequentialCell):
                        input_projection.weight.set_data(initializer(XavierUniform(gain=xavier_std),
                                                                     shape=input_projection.weight.shape,
            ...
            thetas = ops.arange(cell.n_heads, dtype=mindspore.int64).astype(mindspore.float32) * (2.0 * math.pi / cell.n_heads)
            ...
            for i in range(cell.n_points):
                grid_init[:, :, i, :] *= i + 1
            cell.sampling_offsets.bias = Parameter(grid_init.view(-1), 'bias')
            ...
            cell.value_proj.weight.set_data(initializer('xavier_uniform', shape=cell.value_proj.weight.shape, dtype=cell.value_proj.weight.dtype))
            cell.value_proj.bias.set_data(initializer('zeros', cell.value_proj.bias.shape, cell.value_proj.bias.dtype))
            cell.output_proj.weight.set_data(initializer('xavier_uniform', shape=cell.output_proj.weight.shape, dtype=cell.output_proj.weight.dtype))
            ...
            for _, p in cell.parameters_and_names():
                if p.dim() > 1:
                    p.set_data(initializer(XavierUniform(xavier_std), shape=p.shape, dtype=p.dtype))

        elif isinstance(cell, Mask2FormerPixelLevelModule):
            for subcell in cell.cells():
                if isinstance(subcell, (nn.Conv2d, nn.Dense)):
                    subcell.weight.set_data(initializer(Normal(std), shape=subcell.weight.shape, dtype=subcell.weight.dtype))
                    if subcell.has_bias:
                        subcell.bias.set_data(initializer('zeros', shape=subcell.bias.shape, dtype=subcell.bias.dtype))

        elif isinstance(cell, Mask2FormerPixelDecoder):
            for _, p in cell.parameters_and_names():
                if p.dim() > 1:
                    p.set_data(initializer('xavier_uniform', shape=p.shape, dtype=p.dtype))
            cell.level_embed.set_data(initializer(Normal(0), shape=cell.level_embed.shape, dtype=cell.level_embed.dtype))
            ...
            cell.weight.set_data(initializer(Normal(std, 0.0), shape=cell.weight.shape, dtype=cell.weight.dtype))
            if cell.has_bias:
                cell.bias.set_data(initializer('zeros', shape=cell.bias.shape, dtype=cell.bias.dtype))
        ...
        if hasattr(cell, "reference_points"):
            cell.reference_points.weight.set_data(initializer(XavierUniform(1.0), shape=cell.reference_points.weight.shape, dtype=cell.reference_points.weight.dtype))
            cell.reference_points.bias.set_data(initializer('zeros', shape=cell.reference_points.bias.shape, dtype=cell.reference_points.bias.dtype))
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.class_predictor = nn.Dense(
            config.hidden_dim, config.num_labels + 1)

        self.criterion = Mask2FormerLoss(
            config=config, weight_dict=self.weight_dict)
        self.post_init()
